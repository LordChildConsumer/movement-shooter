...
# Make it so the warnings and errors and such are different colors
func _ready() -> void:
	for c in get_parent().colors.values():
		output_box.add_color_region(c["start"], c["end"], c["color"], c["line-only"]);
...
func cycle_command_history(offset: int) -> void:
